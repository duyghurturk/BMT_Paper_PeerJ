...
filt.ps1.1 <- filt.ps1 %>%  subset_taxa(Kingdom == "Bacteria" & Family != "mitochondria" & Class != "Chloroplast")
sample_data(filt.ps1)
sum(is.na(as.data.frame(tax_table(filt.ps1)[,6])$Genus))
length(as.data.frame(tax_table(filt.ps1)[,6])$Genus)
1-9/454
```
...
```{r}
ps1_phylum_nofilter <- filt.ps1 %>% tax_glom(taxrank = "Phylum") %>% psmelt() %>% arrange(desc(Abundance))
ps1_phylum_ra_allsample <- ps1_phylum_nofilter %>%  group_by(Subject) %>%  mutate(rAbundance = Abundance/sum(Abundance)) %>%  filter(rAbundance >= 0.01) %>% ungroup()
ps1_phylum_ra_allsample <- ps1_phylum_ra_allsample %>% mutate(ra_AS = Abundance/sum(Abundance))
ps1_phylum_ra_allsample %>% group_by(Phylum) %>%  select(Phylum,ra_AS) %>% summarize_all(sum) %>% arrange(desc(ra_AS))
```
...
### find out the most abundant taxa at class level across all samples when filtered with >= 1% abundance
```{r}
ps1_Class_nofilter <- filt.ps1 %>% tax_glom(taxrank = "Class") %>% psmelt() %>% arrange(desc(Class)) 
ps1_Class_ra_allsample <- ps1_Class_nofilter %>%  group_by(Subject) %>%  mutate(rAbundance = Abundance/sum(Abundance)) %>%  filter(rAbundance >= 0.01) %>% ungroup()
ps1_Class_ra_allsample <- ps1_Class_ra_allsample %>% mutate(ra_AS = Abundance/sum(Abundance))
ps1_Class_ra_allsample %>% group_by(Class) %>%  select(Class,ra_AS) %>% summarize_all(sum) %>% arrange(desc(ra_AS)) %>%  .[1:10,] 
```
...
### find out the most abundant taxa at family level across all samples when filtered with >= 1% abundance
```{r}
ps1_Genus_nofilter <- filt.ps1 %>% tax_glom(taxrank = "Family") %>% psmelt() %>% arrange(desc(Family)) 
ps1_genus_ra_allsample <- ps1_Genus_nofilter %>%  group_by(Subject) %>%  mutate(rAbundance = Abundance/sum(Abundance)) %>%  filter(rAbundance >= 0.01) %>% ungroup()
ps1_genus_ra_allsample <- ps1_genus_ra_allsample %>% mutate(ra_AS = Abundance/sum(Abundance))
ps1_genus_ra_allsample %>% group_by(Family) %>%  select(Family,ra_AS) %>% summarize_all(sum) %>% arrange(desc(ra_AS)) %>%  .[1:10,] 
```
...
### find out the most abundant taxa at family level across all samples when filtered with >= 1% abundance
```{r}
ps1_Genus_nofilter <- filt.ps1 %>% tax_glom(taxrank = "Genus") %>% psmelt() %>% arrange(desc(Genus)) 
ps1_genus_ra_allsample <- ps1_Genus_nofilter %>%  group_by(Subject) %>%  mutate(rAbundance = Abundance/sum(Abundance)) %>%  filter(rAbundance >= 0.01) %>% ungroup()
ps1_genus_ra_allsample <- ps1_genus_ra_allsample %>% mutate(ra_AS = Abundance/sum(Abundance))
ps1_genus_ra_allsample %>% group_by(Genus) %>%  select(Genus,ra_AS) %>% summarize_all(sum) %>% arrange(desc(ra_AS)) %>%  .[1:10,] 
```

```{r}
GenusCount <- filt.ps1 %>%
  tax_glom(taxrank = "Genus") %>%  psmelt()%>%  arrange(desc(Genus))
sapply(GenusCount, function(x) n_distinct(x))
```
